  ...
  <header class="flex items-center bg-white mx-auto relative px-24 max-[900px]:px-6 h-32">
    <router-link to="/" class="h-fit mr-10 z-50">
      <svg
        ...
        width="108"
        height="29"
        class="hover:fill-yellow fill-dark-navy ease-in duration-200 cursor-pointer"
      >
        <path
    ...
    <!-- Desktop Navigation -->
    <div class="hidden min-[901px]:flex items-center w-full ">
      <Nav :is-mobile="false" :menu-open="false" />
      <Button class="ml-auto" label="Get Scootin" />
    </div>

    <!-- Mobile Hamburger Menu -->
    <button
      @click="isMenuOpen = !isMenuOpen"
      class="flex min-[901px]:hidden ml-auto z-50 w-10 h-10 flex-col justify-center items-center gap-1.5 cursor-pointer"
    >
      <span
        class="block w-6 h-0.5 bg-yellow transition-transform duration-300"
        :class="isMenuOpen ? 'rotate-45 translate-y-2' : ''"
      />
      <span class="block w-6 h-0.5 bg-yellow transition-opacity duration-300" :class="isMenuOpen ? 'opacity-0' : ''" />
      <span
        class="block w-6 h-0.5 bg-yellow transition-transform duration-300"
        :class="isMenuOpen ? '-rotate-45 -translate-y-2' : ''"
      />
    ...
    <!-- Mobile Menu -->
    <div
      class="fixed min-[901px]:hidden top-32 left-0 bottom-0 w-[60%] bg-dark-navy transition-transform duration-300 ease-in-out z-40"
      :class="isMenuOpen ? 'translate-x-0' : '-translate-x-full'"
    >
      <Nav :is-mobile="true" :menu-open="isMenuOpen" @close-menu="isMenuOpen = false" />

      <div class="px-8">
        <Button label="Get Scootin" class="w-full" />
      </div>
    </div>
